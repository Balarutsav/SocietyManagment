...
import com.clotheslyne.customer.R
...
//            ).show()
            ...
            if (msg.isEmpty().not()) {
                val snackbar = Snackbar.make(
                    activity.window.decorView.findViewById(android.R.id.content), validateString(msg), duration
                )
                val snackbarView: View = snackbar.view
                // styling for background of snackbar
                snackbarView.setBackgroundColor(Common.getColor(R.color.white, activity))

                // styling for text color
                snackbar.setTextColor(Common.getColor(R.color.colorPrimary, activity))

                //styling for action of text
                snackbar.setActionTextColor(Common.getColor(R.color.colorPrimary, activity))
                snackbar.show()
...
//    }
    ...
    // for activity and action
    fun show(activity: Activity?, msg: String, actionText: String, clickListener: View.OnClickListener, duration: Int = Snackbar.LENGTH_LONG) {
        if (activity != null && msg.isEmpty().not()) {
            Snackbar.make(activity.window.decorView.findViewById(android.R.id.content), validateString(msg), duration)
                .setAction(actionText, clickListener).show()
        }
    }
    ...
    fun show(view: View?, msg: String, actionText: String, clickListener: View.OnClickListener) {
        if (view != null && msg.isEmpty().not()) {
            Snackbar.make(view, validateString(msg), Snackbar.LENGTH_LONG).setAction(actionText, clickListener).show()
        }
    }
